...
module.exports = async (req, res) => {
  ...
  const { image, prompt, code, type } = req.body;

  if (!image) return res.status(400).json({ error: "No image provided" });

  // Validate license code
  const entry = VALID_CODES[code];
  if (!entry) return res.status(401).json({ error: "Invalid license code" });
  if (entry.expiry && entry.expiry !== "lifetime") {
    if (new Date() > new Date(entry.expiry)) {
      return res.status(401).json({ error: "License expired. Please renew at assignlyai.com/pricing" });
    }
  }
  ...
  try {
    ...
    try {
      ...
      return res.status(200).json({
        question: "Seen on screen",
        answer: text,
        explanation: "",
        submit_hint: "Press Enter",
        has_more: false
      });
    }
  } catch (e) {
